...
- **ホールドアウト検証**: 末尾N日を除いて別途推定し、末尾期間の発見数が予測区間に入るかを評価（最終結果は全データで推定）
...
| `--seed N` | 乱数シード（最適化手法・マルチスタート・ブートストラップ・尤度比検定・適合度検定のブートストラップに使用）。未指定なら設定ファイルの `Bootstrap.RandomSeed`、それもなければ固定値を使い、同じ入力なら同じ結果になります |

- `--coverage`（擬似Coverageモデル、廃止）と `--basic-only` は互換性のため受け付けますが、注意を表示して無視します。
- 上の表にない `-` で始まる引数は無視します。
- 乱数シードは `--seed N` または設定ファイルの `Bootstrap.RandomSeed` で指定します（[ブートストラップ設定](#ブートストラップ設定configjson)）。どちらもなければ固定値を使うので、既定で再現できます。
...
=== 収束判断の目安 ===
  ...
  現在の発見率（点推定）: 95.0% (125 / 131.6)
  現在の発見率（信頼下限）: 84.4%
  判定の根拠: 発見率の信頼下限 84.4%（累積 125 / (累積 + 今後発見される件数の上限 23)、ブートストラップ 95%予測区間）で判定
```
...
| シート | 内容 |
|-------|------|
| データ入力 | 行2〜4 にプロジェクト情報、行6〜10 に分析に使った日次データ（観測期間のみ。空欄は 0）、行12〜16 に累積値（予定消化・実績消化・バグ発生・バグ修正・残存バグ数）、行18〜19 に消化率（%） |
| モデル選択 | 基本モデルの一覧（数式・特徴）、推奨モデルのパラメータ（a・b・c とその他のパラメータ）・適合度（R²・MSE・AIC）・推定結果、モデル比較表（30行目から）、収束予測（90%/95%/99%/99.9%） |
| 予測データ | 1〜2n 日目（n: 観測日数）の実績累積バグ数、予測値 m(t)、潜在バグ総数、残存バグ予測（総数 − m(t)。修正数に基づく「残存バグ数」とは別の量） |

モデル比較表の列は、モデル名・カテゴリ・比較グループ・R²・MSE・AIC・AICc・選択基準・Δ（グループ内）・潜在バグ数・95%/99%発見日（日数）です。`--holdout-days` 指定時は HO予測発見数・HO95%予測区間・HO実測発見数・HO判定・HO誤差(%)・HO日次MAE・損失関数の列が加わります。
収束予測の欄には、現在の発見率（点推定と信頼下限）と★評価・判定の根拠を出力します。信頼区間・予測区間・推定の安定性・警告・統計診断・モデル平均化は Excel には出力しません。有限工数の TEF モデルが推奨された場合、推定結果の見出しは「推定総工数で見つかるバグ数」になります。
...
推奨モデルの**現在の発見率の信頼下限**から、収束状況の目安を表示します。

```text
発見率の信頼下限 = 累積発見数 / (累積発見数 + 今後発見される件数の予測区間の上限)
```

| 表示 | 条件（信頼下限） | メッセージ |
|:----:|------|-----------|
| ★★★ | ≥ 99% | 十分に収束しています。リリース可能な状態です。 |
| ★★☆ | ≥ 95% | ほぼ収束しています。ベータリリースに適した状態です。 |
| ★☆☆ | ≥ 90% | 収束傾向にあります。継続的なテストが推奨されます。 |
| ☆☆☆ | < 90%、または下限が定まらない | まだ収束していません（収束していると判断できません）。テスト継続が必要です。 |
| －－－ | 推定潜在バグ総数が算出できない（非有限・0 以下） | 推定潜在バグ総数を算出できないため、収束判断できません。 |

- **今後発見される件数の予測区間**（パラメータ推定の不確実性 + Poisson 変動）は、`--ci`/`--pi` でブートストラップを実行した場合はその予測区間、しなかった場合は Fisher 情報行列（デルタ法、変化点 τ は固定）と Poisson・対数正規の混合で求めます（計算は安いので常に行います）。FRE・TEF モデルや SSE で推定した場合は区間を求められないため、点推定の発見率（累積発見数 ÷ 推定潜在バグ総数 m(∞)）で判定し、その旨を表示します。
- 次の場合は発見率の下限が定まらないので、**☆☆☆ 止まり**にして理由を表示します。
  - 今後発見される件数の上限が探索範囲で決まっている（「≥」）、または本推定の a が上限に張り付いている
  - [推定の安定性](#推定の安定性データ打ち切りに対する感度)が「不安定」（末尾の数日を除くと推定総数が大きく変わる）
- 点推定の発見率・信頼下限・判定の根拠をあわせて表示します。発見率（点推定）が 100% を超える（推定総数が累積発見数を下回る）場合は、「モデルがデータに適合していない可能性があります」という注意を追加します。
- コマンドラインでは★を色分け（緑・シアン・黄・赤）して表示します。

> **点推定で判定しない理由**: 点推定の発見率は推定のばらつきが大きく、真の発見率 83.5%（指数型、a = 60、40 日）の同じ条件のデータ 100 セットで 0.46〜0.96 に散らばり、点推定で判定すると 24 セットに ★☆☆ 以上が付いていました。信頼下限で判定すると 0 セットです。
>
> **総数 m(∞) の信頼区間の上限で割らない理由**: a は総数の Poisson 分布の平均なので、ほぼすべて発見済みでも区間の幅が √a 程度残ります（真の発見率 99.97% のデータでも「累積 ÷ 総数の上限」は 86% 程度にしかならず ★ が付きません）。発見率に効くのはまだ見つかっていない件数なので、その予測区間を使います。真の発見率 99.97%（a = 150）のデータ 100 セットでは 96 セットが ★★★ です。
>
> **小規模なプロジェクトでは厳しめになります**: 総数が 100 件未満だと、今後発見される件数の上限が 1 件でも発見率の下限は 99% を下回るため、★★★ には「今後発見される件数の上限 0 件」が必要です。
...
| 変曲S字型（Ohba） | m(t) = a(1 - e^(-bt)) / (1 + ψ·e^(-bt))、ψ = e^(lnψ) | ψ→0で指数型。ψが大きいほど立ち上がりの遅い S 字。切断ロジスティックと同一 |
...
> - **ロジスティック**は削除しました。ロジスティック曲線 L(t) = 1/(1+e^(-b(t-c))) を t ≥ 0 に切断・正規化すると、ψ = e^(bc) の変曲S字型と恒等的に等しくなるためです（変曲点は t* = ln(ψ)/b で、ロジスティックの c にあたります）。以前の `a/(1+e^(−b(t−c)))` は m(0) ≠ 0 で、観測開始前に発見済みの件数を総数に含めてしまうため、そのままは戻していません。
> - **変曲S字型の ψ は ln ψ で推定します**（探索範囲 [−10, 20]、初期値 0）。以前は ψ ∈ [0, 1000] で、急峻で変曲点の遅い S 字（ψ = e^(b·t*) が数万〜数百万）を表せず、ψ が上限に張り付いて総数を過大に推定していました（ψ ≈ 3.6 万・真の総数 100 の合成データ 50 セットで 50/50 回張り付き。ln ψ にすると張り付き 0 回）。ln ψ の下限 −10 は実質的に指数型と同じなので、張り付いても注意しません。推定結果には変曲点 t* = ln ψ / b（日。ln ψ > 0 のとき）も表示します。
...
| | b | [0.001, 1.0] | 発見率 |
| | lnψ | [−10, 20] | 変曲パラメータ ψ の対数（変曲点 t* = lnψ / b。下限で実質的に指数型） |
...
| 変曲S字型+変化点 | m(t) = a(1-e^(-u(t)))/(1+ψ·e^(-u(t)))、ψ = e^(lnψ) | 変曲S字型の検出率が τ で変化（API から利用可能） |
...
この方式により m(t) は変化点 τ の前後で連続となります。検出強度 dm/dt は τ で b₁ 側から b₂ 側へ跳びます（b₁ = b₂ のときだけ連続。変化点としてはこれが正しい挙動です）。
...
TEF には指数型・Rayleigh型・Weibull型・ロジスティック型・べき乗則を実装しており、`--tef` では Weibull型 TEF を使用します。有限工数の TEF（総工数 N）では m(∞) = m(W=N) で、a より小さくなります。これは潜在バグ総数ではなく「推定した総工数 N を使い切るまでに見つかる数」なので、出力では「推定総工数で見つかるバグ数」と表示します。
...
発見数 m_d(t) と修正数 m_c(t) を同時に推定するモデルです（`--fre`。修正数データ「バグ修正」が必要）。発見されたバグのうち割合 η が、発見から平均 D 日（指数分布）遅れて修正されるとします。

```text
m_c(t) = ∫₀ᵗ η(s)·m_d'(s)·G(t−s) ds,   G(u) = 1 − e^(−u/D)
```

D = 0 なら発見と同時に修正され、m_c(t) = η·m_d(t) になります。

| モデル名 | 数式 |
|---------|------|
| 定数FRE | m_d(t) = a(1-e^(-bt))、m_c(t) = η[m_d(t) − ab·K(b, 1/D, t)] |
| 学習FRE | 除去効率が η(t) = η∞ − (η∞−η₀)e^(−λt) で向上（m_c は上の畳み込み） |
| ロジスティックFRF | 欠陥削減係数が FRF(t) = 1/(1+β·e^(−γt)) で変化（API から利用可能） |
| FRE+変化点 | m_d(t) = a(1-e^(-u(t)))、m_c(t) は上の畳み込み（API から利用可能） |

K(β, μ, t) = (e^(−βt) − e^(−μt)) / (μ − β)。いずれも閉形式で計算します。`--fre` で実行されるのは定数FRE と学習FRE です。

> **修正遅れ D を加えた理由**: 以前は m_c(t) = η·m_d(t)（発見と同時に修正）で、修正が遅れるデータでは修正数の立ち上がりの遅れを検出率 b の小ささで説明してしまい、a が過大・b が過小に偏っていました。真値 a = 200、b = 0.04、η = 0.9、40 日のバグ単位のシミュレーション（各 50 データ）で、平均 3 日遅れでは a +11%・b −16%、7 日遅れでは a +31%・b −36% でした（発見数だけの指数型では a の偏りはほぼ 0）。D を加えると a ±1%・b +5% 程度になり、D もほぼ真値（3.1 日・7.1 日）に推定されます。遅れのないデータでは D ≈ 0 と推定されます。

| モデル | パラメータ | 探索範囲 |
|--------|-----------|---------|
| **定数FRE** | a, b | [maxY, maxY×5], [0.001, 1.0] |
| | η | [0.3, 1.0] |
| | D | [0, n]（0 は自然な境界として張り付いても注意しない） |
| **学習FRE** | a, b | [maxY, maxY×5], [0.001, 1.0] |
| | η₀, η∞ | [0.1, 0.9], [0.7, 1.0] |
| | λ | [0.01, 1.0] |
| | D | [0, n] |
| **FRE+変化点** | a, b₁, b₂ | [maxY, maxY×5], [0.001, 1.0], [0.001, 1.0] |
| | η, D, τ | [0.3, 1.0], [0, n], [2, n−2] |
| **ロジスティックFRF** | β, γ | [0.5, 50], [0.01, 2.0] |

> 発見数と修正数は同じバグの過程なので、修正数を独立な Poisson とみなす結合尤度は厳密な尤度ではなく複合尤度です（推定値は一致性を保ちますが、AIC は近似です）。
...
| 95%予測区間 / 判定 | 末尾期間の発見数の予測区間と、実測がその内か外か（警告はこれで判定） |

予測発見数はモデル自身の m(T_train) を起点に計算するため、訓練最終時点での当てはまりのずれ（水準のずれ）は評価に含めません。

**予測が外れているかは予測区間で判定します**。末尾期間の発見数は、正しいモデルでも Poisson 分布でばらつき、さらに訓練区間の推定にも不確実性があります。そこで予測分布を X ~ Poisson(Λ)、ln Λ ~ N(ln Δ̂, s²)（s は訓練区間の Fisher 情報行列とデルタ法による ln Δ̂ の標準誤差。変化点 τ は固定）の混合とし、実測が 95% 予測区間の外にあるときだけ警告します。Fisher 情報行列を計算できない場合は Poisson 変動だけの区間を使います（その旨を表示）。

以前は HO誤差の絶対値 30%・60% の固定しきい値で警告していましたが、件数の少ない末尾期間では Poisson 変動だけで誤差が大きくなり、正しいモデル（指数型データに指数型、40 日、200 データ）でも末尾 5 日で 60%、10 日で 41% に警告が出ていました。予測区間による判定では 2.5〜4% です（遅延S字型データに指数型を当てはめた場合は、10 日で 72% を区間外と判定します）。HO誤差・日次MAE は参考として表示します。
...
- **コンソール**: モデル比較表に「HO誤差(%)」列を追加し、HO誤差の絶対値が最小のモデルと推奨モデルの予測（予測区間）・実測発見数、予測区間の外にあるモデルを表示
- **Excel**: `HO予測発見数`, `HO95%予測区間`, `HO実測発見数`, `HO判定`, `HO誤差(%)`, `HO日次MAE` 列を追加
- **テキストレポート**: 「ホールドアウト検証結果」セクションに全モデルの予測発見数・予測区間・実測発見数・判定・誤差・日次MAE/RMSE を出力
...
| 末尾期間の実測発見数が 95% 予測区間の外 | モデルの将来予測は信頼できない可能性（過大・過小予測の向きも表示） |
| 上に加えて、選択モデルのHO誤差の絶対値が全モデルの平均の1.5倍超かつ最小の2倍超 | 予測性能が低い可能性 |
| 上に加えて、選択モデルのHO誤差の絶対値が HO誤差最小のモデルの1.3倍超かつ5ポイント以上大きい | 予測精度を重視するなら HO誤差最小のモデルも検討 |
...
| その他のパラメータが探索範囲の境界に張り付き | 推定値の解釈に注意（境界が 0 の場合・η = 1・ln ψ の下限は、別のモデルに一致する自然な境界なので除く） |
| ブートストラップの再推定で a が上限に張り付いた割合が上側の裾を超える | 区間の上限は探索範囲で決まっており、実際はそれ以上でありうる（「≥」で表示） |
...
- **収束判定**: 収束条件を満たす前に最大反復回数に達した場合は、推定結果に警告を付けます。最良値が更新されなくなっただけ（停滞）では「収束」とみなしません。
- **乱数シード**: DE・PSO・GWO・CMA-ES とマルチスタートの開始点は、`--seed`（未指定なら設定の `Bootstrap.RandomSeed`、それもなければ固定値）から推定ごとに決まるシードを使います。推定ごとのシードは基準シード・モデル名・データの内容から作るので、ブートストラップなどで並列に推定しても、同じ入力なら同じ結果になります（以前は最適化手法にシードを渡しておらず、同じデータでも実行ごとに推定値が揺らいでいました）。
...
この表の後、CMA-ES と GWO をさらに修正しました（下記）。CMA-ES は収束の遅い指数型データなどで 100 回中最大 7 回（シードを変えて実行）局所解・途中の解を「収束」として返していましたが、0 回になりました。GWO は SSE で 30 回中 30 回、最適値に届かないまま「収束」と報告していましたが、最適値（相対差 1e-6 以内）に到達するようになりました。
...
GWO は探索係数 a を 2→0 に減らして大域探索から局所探索に移ります。最良値の停滞では打ち切らず、α・β・δ の位置が探索範囲の 1e-8 以内にそろったときだけ収束とします。群れは終盤まで縮みきらず最良解の精度が低い（相対誤差 1e-5〜1e-2 程度残る）ため、最後に α から Nelder-Mead 法で仕上げ、収束の判定もその結果に従います。以前は最良値が 50 反復更新されないだけで打ち切って Converged=true としており、SSE では探索の途中で止まっていました。

#### CMA-ES（共分散行列適応進化戦略）

| パラメータ | デフォルト値 | 説明 |
|-----------|-------------|------|
| maxIterations | 500 | 1回の探索の最大世代数（全体の評価回数の上限は maxIterations × 初期 λ × 4） |
| initialSigmaU | 0.5 | u空間での初期ステップサイズ |
| tolerance | 1e-10 | 適応度の幅の収束判定閾値（相対） |
| λ | max(6, 4 + floor(3·ln(n))) | 1回目の探索の個体数（自動計算。やり直すたびに倍） |
| μ | λ / 2 | 選択個体数（自動計算） |

- ロジスティック変換により無拘束空間で最適化します。初期点が境界上（ψ=0、a=maxY など）でもシグモイドが飽和しないよう、初期点の u を ±4 に丸めます。
- **IPOP-CMA-ES**: 1回の探索が収束または停滞したら、個体数 λ を倍にして探索範囲内のランダムな点からやり直します（最大 8 回）。2回の探索が同じ最良値に収束したらやめます。
- **収束判定**: 最良解を出した探索が、u空間の探索幅（TolX、1e-9 未満）または適応度の幅（TolFun）の基準で止まったときだけ収束とします。最良値の停滞はやり直しのきっかけで、収束とはみなしません（以前は最良値が 51 世代更新されないだけで収束とし、やり直しもしなかったため、局所解をそのまま返していました）。共分散行列の分解に失敗した場合も、単位行列に戻さずにやり直します。
...
**上限が探索範囲で決まっている場合（「≥」表示）**: 本推定の a が上限に張り付いていなくても、再推定の一部では a が探索範囲の上限（その合成データの最大累積数の 5 倍）に張り付くことがあります。張り付いた反復の「本当の」推定値は上限より大きいので、その割合が上側の裾（95% 区間なら 2.5%）を超えると、上側の分位点は探索範囲で決まった値にすぎません。この場合は総数・今後発見される件数・収束日（と将来の m(t)・累積発見数）の上限を「≥ 1190.0」のように表示し、警告します。張り付いた反復は除外しません（除くと区間が不当に狭くなります）。例: 観測最大値 238 件・推定総数 624 件のデータで、再推定の 1 割強が上限に張り付いていました（以前は警告もなく普通の区間として表示していました）。
...
| `RandomSeed` | null | 乱数シード（`--seed` で上書き。両方ないときは固定値）。最適化手法・ブートストラップ区間・変化点の尤度比検定・適合度検定のブートストラップに使用 |
...
- **対象外のモデル**: 修正数データを同時に推定する FRE モデルは対象外です（発見数だけを再生成すると修正数との整合が取れないため）。
- **TEF モデル**: 工数も発見数と同時に（正規分布の尤度で）推定しているので、工数も推定した工数関数から Ŵ(tᵢ) + N(0, σ̂²) で再生成し、工数を差し替えたモデルの複製で推定し直します（以前は工数を観測値に固定しており、工数関数のパラメータがほとんど動かず、工数の不確実性が区間に入りませんでした）。なお工数の尤度は累積工数の各日の誤差を独立とみなすため、総工数 N などの不確実性は小さめに出ます。

## Fisher情報行列による解析的信頼区間

`--ci` を MLE（デフォルト）で実行すると、ブートストラップに加えて、最尤推定量の漸近理論に基づくパラメータと推定潜在バグ総数の信頼区間を計算します。

- 変化点モデルは、尤度が τ について微分できないため、τ を固定した条件付きの区間を計算します（τ の不確実性は含みません）。
...
- 変曲S字型の ln ψ のように、下限付近で尤度がほぼ平らになるパラメータは、元の座標（ψ）でヘッセ行列を求め、ヤコビアンで推定に使う座標の共分散に戻します。
- `--ci` を指定しなくても、[★評価](#収束判断の目安★評価)と[ホールドアウトの予測区間](#ホールドアウト検証)のために内部で計算します（表示はしません）。
...
**ランダム化分位残差**に対して行い、**Ljung-Box 検定だけで判定**します（p < 0.05 で「自己相関あり」）。Durbin-Watson と各ラグの自己相関係数は参考として表示します。

| 検定名 | 帰無仮説 | 使い方 |
|--------|---------|-------------|
| Ljung-Box Q | ラグkまで自己相関なし（k = min(10, n/4)） | 判定に使用（p値 > 0.05 で帰無仮説採択） |
| Durbin-Watson | 1次自己相関なし | 参考（2に近いほど良好） |
| 各ラグの ACF | ラグごとに自己相関なし | 参考（±1.96/√n。ラグの数だけ偶然に超えやすい） |

以前は Pearson 残差に対して「Ljung-Box が有意、または DW が 1.5〜2.5 の外、または 10 ラグのどれかが ±1.96/√n の外」の OR で判定しており、正しいモデルでも 28〜35% を自己相関ありとしていました（10 ラグの個別判定だけで約 40% になります）。今の判定では正しいモデル（指数型データに指数型、40 日、200 データ）で 6.5〜9% です。
...
正規性検定も**ランダム化分位残差**に対して行います。日次発見数は離散で期待値が小さいことが多く、Pearson 残差は正しいモデルでも正規分布に従わないためです（以前は Pearson 残差に適用しており、正しいモデルでも棄却されやすくなっていました）。ランダム化分位残差はモデルが正しければ厳密に標準正規分布に従います。
...
判定に使える検定の過半数で棄却されなければ「適合」と判定します（3 つとも使えるときは 2 つ以上のパス）。

- **KS・CvM を判定に使わない場合**: 再推定の成功が半数未満の場合と FRE モデル（修正数を同時に推定するため発見数だけの再推定では整合しない）では、KS・CvM の p 値はパラメータ既知を仮定した漸近分布で求めます。この p 値は正しいモデルでもほとんど棄却しない（棄却率ほぼ 0%）ため、参考表示にとどめ、判定は χ² 検定だけで行います（以前は多数決に入れており、χ² で棄却されても「適合」になりえました）。
- **χ² の自由度**: 各ビンの件数は独立な Poisson 変数で、合計は固定されない（多項分布ではない）ため、自由度は「ビン数 − 発見数の m(t) に効くパラメータ数」です（以前は −1 余分に引いていました）。FRE モデルの η・D のように修正数にしか効かないパラメータは数えません。自由度が残らない（ビン数 ≤ パラメータ数）場合は検定できないとし、判定にも使いません（以前は自由度 1 として p 値を出していました）。
- **残差の種類**: 表示する残差分析は Pearson 残差です。採点（系統的パターン・外れ値・歪度・尖度）・自己相関・正規性の検定はランダム化分位残差で行います。ランダム化分位残差の乱数シードは固定です。
...
**検定の大きさと検出力**（合成データ、40日、有意水準 5%、各 1000 データ。指数型データは a=150, b=0.05、遅延S字型データは a=150, b=0.1。既定の差分進化・最尤推定・シード固定）:
...
| KS | 4.2% | 89.0% |
| CvM | 4.7% | 94.5% |
| 総合判定（「不適合」の割合） | 3.5% | 90.3% |

χ² 検定はパラメータを（ビンにまとめる前の）日次データで最尤推定しているため、統計量の分布は自由度「ビン数 − パラメータ数」と「ビン数」の χ² 分布の間にあり（Chernoff–Lehmann）、自由度「ビン数 − パラメータ数」の p 値はやや棄却しやすくなります（名目 5% に対し 6.3%）。以前の README の 3.0% は再現できませんでした。

### 診断グレード

診断結果は0〜100のスコアと5段階のグレードで評価されます。スコアは 100 点から、次の減点をして求めます。減点はすべて**ランダム化分位残差**で判定します。

| 項目 | 減点 |
|------|------|
| 系統的パターン（ラン検定 p < 0.05、または全残差が同符号） | −30 |
| 外れ値（\|r\| > 3）の割合 | 5% 超で −8、10% 超で −15 |
| 歪度・尖度（標準誤差 √(6/n)・√(24/n) の何倍か） | それぞれ 2 倍超で −5、3 倍超で −10 |
| 自己相関（Ljung-Box） | p < 0.05 で −10、p < 0.01 で −20 |
| 正規性の棄却 | −5（Jarque-Bera・Anderson-Darling とも p < 0.01 なら −10） |

以前はパターン・外れ値・歪度・尖度を Pearson 残差で判定し、歪度・尖度のしきい値も固定（0.5・1.0 など）でした。日次発見数は離散で期待値が小さいため、正しいモデルでも |歪度| > 0.5 が 5〜8 割になり、減点されやすくなっていました。適合度検定と Poisson 整合性診断はスコアに含みません。
...
═══════════════════════════════════════════════════════════════
  ...
  診断日時: 2026-09-26 00:23:09
═══════════════════════════════════════════════════════════════

【総合評価】モデル「Goel一般化（Weibull型）」の診断結果: 許容範囲（スコア: 60/100）。このモデルの予測は一定の信頼性を持ちます。
...
─────────────────────────────────────────────────────────────────
  ランダム化分位残差（採点・自己相関・正規性の検定に使用）
─────────────────────────────────────────────────────────────────
  ※ 日次発見数は離散のため Pearson 残差は正しいモデルでも歪む。採点はモデルが正しければ標準正規分布に従うこの残差で行う
  歪度: 0.0484
  尖度: -1.1499
  外れ値（|r| > 3）: 0点
  ラン検定: 観測=17, 期待=10.9, p=0.0046
...
─────────────────────────────────────────────────────────────────
  自己相関検定（判定は Ljung-Box 検定。DW・各ラグは参考）
─────────────────────────────────────────────────────────────────
  Durbin-Watson（参考）: 3.1742
    → 強い負の自己相関（過剰適合または外れ値の影響）
  Ljung-Box Q: 11.1677 (df=5, p=0.0482)
  ±1.96/√n を超えたラグ（参考。ラグの数だけ偶然に超えやすい）: 1
  判定: ⚠ 自己相関あり
...
─────────────────────────────────────────────────────────────────
  ...
  ランダム化分位残差（Dunn & Smyth 1996）: 平均=0.006, 標準偏差=0.586
    （モデルが正しければ標準正規分布に従う。平均≈0、標準偏差≈1 が目安）
...
【適合度検定】モデル「Goel一般化（Weibull型）」はすべての適合度検定をパスしました。データに良く適合しており、予測の信頼性は高いと考えられます。
  ...
  KS検定: D=0.0289 (p=0.9350)
  CvM検定: W²=0.0115 (p=0.9850)
...
修正数データを同時に推定する FRE モデルは対象外です。TEF モデルは工数も再生成します（[信頼区間の注意事項](#注意事項)）。上限が探索範囲で決まっている場合は「≥」を付けて表示します。

「今後発見される件数」の予測区間は、`--pi` を指定しなくてもブートストラップを実行した場合（`--ci`）は[★評価](#収束判断の目安★評価)に使います。
...
パラメータ推定の不確実性を計算できない場合（SSE 推定の場合や、FRE・TEF モデルを含む場合）は、「無条件標準誤差」の代わりに「モデル間標準偏差」とその旨の注記を表示します。変化点モデルは τ を固定した条件付きの分散で含めます（τ の不確実性は含みません）。
