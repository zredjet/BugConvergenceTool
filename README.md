...
| `--loss TYPE` | 損失関数を指定（mle/sse、デフォルト: mle） |
...
> **注意**:
> - **TEFモデル**を使用する場合は、「予定消化」および「実績消化」の行に有効な工数データが必要です。
...
行7     |  20  |  25  |  25  |  30  | ...  ← 予定消化（日次） ※TEFモデルで必須
行8     |  15  |  22  |  28  |  25  | ...  ← 実績消化（日次） ※TEFモデルで必須
行9     |   8  |  12  |  15  |  10  | ...  ← バグ発生（日次）
行10    |   2  |   5  |   8  |  10  | ...  ← バグ修正（日次） ※FREモデルで必須
```

- **観測期間**: 「バグ発生」（行9）が入力されている最後の日までを観測期間とします。予定消化だけを先の日付まで入力しておいても、未来の日は「バグ 0 件の観測日」として扱いません（以前は日付が続く限り読み込み、空欄を 0 件としていました）。観測期間内の空欄は 0 件として扱い、注意を表示します。
...
|---------|:-------------:|:---------------:|------|
| **プロジェクト情報** | ○ 基本情報 | ◎ 詳細 | CLIは簡略版 |
| **現在の状況（消化率、累積バグ等）** | ◎ | ◎ | 同等 |
| **モデル比較結果** | ◎ | ◎ | 同等 |
| **推奨モデル詳細（パラメータ）** | △ `-v`時のみ | ◎ | CLIは`--verbose`で表示 |
| **適合度指標（AIC/AICc等）** | ○ AICのみ | ◎ 全指標 | CLIは主要指標のみ |
| **収束予測** | ◎ | ◎ | 同等 |
| **収束判断の目安（★評価）** | ◎ | ◎ | 同等 |
| **推定の安定性** | ○ 1行 | ◎ 表 | CLIは判定と総数の範囲のみ |
| **変化点探索結果** | - | ◎ | 詳細すぎるためレポートのみ |
...
> **推奨**: 複雑なモデル（変化点、FRE等）でMLEを使用する場合、計算安定性のため `--optimizer de` または `--optimizer cmaes` の指定を推奨します。勾配法ベースのオプティマイザでは収束しない可能性があります。
...
- **モデル平均化**も推奨モデルと同じグループのモデルだけに AIC 重みを付けます。
...
- 検定するのは、選択基準値（AICc）が同じ比較グループの変化点なしモデルの最良値より小さい変化点モデルだけです（それ以外はもともと推奨されないため）。
...
変化点 τ は帰無仮説の下で識別できない局外パラメータとなるため、通常のχ²分布による近似は不正確です（χ² 近似の p 値は参考値として表示）。
...
| 選択モデルのHO誤差が他より明らかに大きい | 予測性能が低い可能性 |
| R² < 0.9 | データへの当てはまりが不十分 |
| パラメータ a が探索範囲の上限に張り付き | 潜在バグ総数を推定できていない（**推奨対象外**） |
| その他のパラメータが探索範囲の境界に張り付き | 推定値の解釈に注意 |
...
同じ比較グループのすべてのモデルが推奨対象外の場合は、選択基準値が最小のモデルを表示したうえで、結果が参考値である旨を警告します。モデル平均化も推奨対象のモデルだけに重みを付けます。
...
1. **初期点の生成**
   - モデルの初期推定値、LHS で探索空間を均等に分割した点、中央点、境界付近（各次元の 10%/90% 点）の点
2. **並列最適化**
   - 各初期点から独立に最適化を実行（最適化器は開始点ごとに生成）し、最良解（最小損失値）を採用
3. **結果の報告**
   - 最良解と同じ目的関数値（相対差 1e-6 以内）に到達した開始点の数を記録し、半数未満なら「解が初期点に依存している可能性」を警告
...
| パラメータ | 説明 |
|-----------|------|
| `P0` | 未使用（不完全デバッグモデル削除のため。互換性のため読み込みのみ） |
| `Eta0` | 初期欠陥除去効率η₀（デフォルト: 0.8） |
| `EtaInfinity` | 漸近欠陥除去効率η∞（デフォルト: 0.95） |
| `Alpha0` | 未使用（バグ混入率 α のモデル削除のため。互換性のため読み込みのみ） |
...
`--pi` と同時に指定した場合、ブートストラップは1回だけ実行して両方に使います。
...
| `RandomSeed` | null | 乱数シード（null で毎回異なる。再現性が必要な場合に指定） |
...
=== パラメータの信頼区間（Fisher情報行列、95%）===
  ...
  推定潜在バグ総数: 137.4 件  [103.8, 181.9]（デルタ法・対数スケール）
```
...
| Kolmogorov-Smirnov | 分布全体 | パラメトリック・ブートストラップ（199回） |
| Cramér-von Mises | 分布全体（二乗誤差） | パラメトリック・ブートストラップ（199回） |
...
| グレード | スコア | 意味 |
|---------|-------|------|
| Excellent | 85〜100 | モデル仮定が十分に満たされている |
| Good | 70〜84 | 軽微な問題があるが実用上問題なし |
| Acceptable | 55〜69 | 注意が必要だが使用可能 |
| Caution | 40〜54 | モデル選択の再検討を推奨 |
...
修正数・工数データを含むモデル（FRE・TEF）は対象外です。
...
```text
  推定総バグ数（モデル平均化）
  平均: 127.3
  無条件標準誤差: ±19.3（Burnham & Anderson）
    パラメータ推定の不確実性: ±17.3（分散の 81%）
    モデル選択の不確実性:     ±8.4（分散の 19%）

  収束予測（モデル平均化）
  80%: 日37 (モデル間 ±6日, 5モデル)
  90%: 日51 (モデル間 ±10日, 5モデル)
  95%: 日65 (モデル間 ±14日, 5モデル)
  99%: 日97 (モデル間 ±23日, 5モデル)
```

### モデル平均化の注意点

- 重みを付けるのは、推奨モデルと同じ比較グループの、推奨対象外でないモデルだけです
