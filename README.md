...
- **観測期間**: 「バグ発生」（行9）が入力されている最後の日までを観測期間とします。予定消化だけを先の日付まで入力しておいても、未来の日は「バグ 0 件の観測日」として扱いません（以前は日付が続く限り読み込み、空欄を 0 件としていました）。観測期間内の空欄は 0 件として扱い、注意を表示します。`=IF(...,"")` のように空文字を返す数式セルも未入力とみなします。
