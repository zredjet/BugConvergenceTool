...
#### Phase 1: メタヒューリスティック最適化

従来のグリッドサーチ+勾配降下法に加え、以下のアルゴリズムを実装：

| アルゴリズム | 特徴 |
|-------------|------|
| **DE（差分進化）** | 非微分可能・マルチモーダル関数に強い（デフォルト推奨） |
| **PSO（粒子群最適化）** | 群知能に基づく大域的最適化 |
| **GWO（Grey Wolf Optimizer）** | オオカミの群れ行動に基づく最適化 |
| **AutoSelect** | 全アルゴリズムで比較し最良を自動選択 |

#### Phase 2: テスト工数関数（TEF）モデル

テスト工数の分布を考慮したモデル：

- 指数型TEF: 工数消費率が単調減少
- Rayleigh型TEF: ピークのある工数消費
- Weibull型TEF: 汎用（形状可変）
- ロジスティック型TEF: 初期工数がゼロでない場合
- べき乗則TEF: 無限工数対応

#### Phase 3: 変化点（Change Point）モデル

テスト環境変化による欠陥検出率の変化を考慮：
